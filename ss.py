...
def display_cloud(cloud: PointCloudLMS):
    ...
    print(
        f"Puntos: {num_points}, ángulo inicio: {cloud.start_angle:.2f}°, fin: {cloud.end_angle:.2f}°, Δ: {cloud.ang_increment:.5f}°"
    )
    max_to_print = 10
    ...
    for idx in range(limit):
        ...
        print(f"{idx}: x={x:.3f} m, y={y:.3f} m, r={r:.3f} m, I={inten}")
...
def main():
    ...
    try:
        ...
        print(f"No se pudo conectar al escáner en {SCANNER_IP}:{SOPAS_PORT}. Detalle: {ex}")
        return
        ...
        while True:
            try:
                ...
                if idle_timeouts >= MAX_IDLE_TIMEOUTS:
                    print("No se recibieron datos del escáner por un tiempo prolongado. Saliendo…")
                    break
                else:
