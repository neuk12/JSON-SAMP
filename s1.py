...
def display_cloud(cloud: PointCloudLMS, source_label: str = ""):
    prefix = f"[{source_label}] " if source_label else ""
    points = cloud.cartesian_2d()
    num_points = points.shape[1]
    print(
        f"{prefix}Puntos: {num_points}, ángulo inicio: {cloud.start_angle:.2f}°, fin: {cloud.end_angle:.2f}°, Δ: {cloud.ang_increment:.5f}°"
    )
    max_to_print = 10
    limit = min(max_to_print, num_points)
    for idx in range(limit):
        x = points[0, idx]
        y = points[1, idx]
        r = cloud.ranges[idx]
        inten = int(cloud.intensities[idx])
        print(f"{prefix}{idx}: x={x:.3f} m, y={y:.3f} m, r={r:.3f} m, I={inten}")
...
SCANNER_IP = "192.168.95.194"  # IP por defecto si no se pasa por argumentos
SOPAS_PORT = 2111

CONNECT_TIMEOUT_S = 3.0
RECV_TIMEOUT_S = 2.0
MAX_IDLE_TIMEOUTS = 10  # ~20s sin datos


def send_command(scanner_sock: socket.socket, cmd: str):
...
def run_scanner(scanner_ip: str):

	# Conexión TCP al escáner con timeout y manejo de error
	try:
		scanner_sock = socket.create_connection((scanner_ip, SOPAS_PORT), timeout=CONNECT_TIMEOUT_S)
		scanner_sock.settimeout(RECV_TIMEOUT_S)
		print(f"[{scanner_ip}] Conectado")
	except OSError as ex:
		print(f"[{scanner_ip}] No se pudo conectar al escáner en {scanner_ip}:{SOPAS_PORT}. Detalle: {ex}")
		return
	...
	send_command(scanner_sock, accessmode_cmd)

	ntp_role_cmd = "\x02sWN TSCRole 1\x03"
	send_command(scanner_sock, ntp_role_cmd)

	ntp_iface_cmd = "\x02sWN TSCTCInterface 0\x03"
	send_command(scanner_sock, ntp_iface_cmd)

	ntp_server_cmd = "\x02sWN TSCTCSrvAddr " + " ".join(ip_hex_digits) + "\x03"
	send_command(scanner_sock, ntp_server_cmd)

	set_scancfg_cmd = "\x02sMN mLMPsetscancfg +2500 +1 +1667 -50000 +1850000\x03"
	send_command(scanner_sock, set_scancfg_cmd)

	set_scandatacfg_cmd = "\x02sWN LMDscandatacfg 00 00 1 0 0 0 00 0 0 0 1 +1\x03"
	reply = send_command(scanner_sock, set_scandatacfg_cmd)

	set_echo_cmd = "\x02sWN FREchoFilter 2\x03"
	reply = send_command(scanner_sock, set_echo_cmd)

	set_outputrange_cmd = "\x02sWN LMPoutputRange 1 +1667 -50000 +1850000\x03"
	reply = send_command(scanner_sock, set_outputrange_cmd)

	storeparams_cmd = "\x02sMN mEEwriteall\x03"
	reply = send_command(scanner_sock, storeparams_cmd)

	run_cmd = "\x02sMN Run\x03"
	reply = send_command(scanner_sock, run_cmd)
	stop_cmd = "\x02sMN LMCstopmeas\x03"
	reply = send_command(scanner_sock, stop_cmd)

	senddata_cmd = "\x02sEN LMDscandata 1\x03"
	reply = send_command(scanner_sock, senddata_cmd)
	...
	try:
		while True:
			try:
				...
				if idle_timeouts >= MAX_IDLE_TIMEOUTS:
					print(f"[{scanner_ip}] No se recibieron datos del escáner por un tiempo prolongado. Saliendo…")
					break
				else:
					print(f"[{scanner_ip}] Esperando datos del escáner…")
					continue

			idle_timeouts = 0  # llegó algo, resetear contador
			...
			if etx_idx >= 0:
				...
				if c % 2 == 0:
					display_cloud(cloud, source_label=scanner_ip)
				partial_datagrams = list()
				partial_datagrams.append(data[etx_idx + 1 :])
			else:
		...
		print(f"[{scanner_ip}] Interrumpido por el usuario")
	except OSError as ex:
		print(f"[{scanner_ip}] Error de socket: {ex}")
	finally:
		try:
			stop_cmd = "\x02sMN LMCstopmeas\x03"
			send_command(scanner_sock, stop_cmd)
		except Exception:
			pass
		try:
			scanner_sock.close()
...
def main():
	...
	parser = argparse.ArgumentParser(description="Lectura de SICK LMS5xx por SOPAS")
	parser.add_argument("--ip", action="append", dest="ips", help="IP del láser (usar varias veces para múltiples láseres)")
	args = parser.parse_args()




	ips = args.ips if args.ips else [SCANNER_IP]
	threads = []
	for ip in ips:
		th = threading.Thread(target=run_scanner, args=(ip,), daemon=True)
		th.start()
		threads.append(th)

	try:
		# Mantener el proceso vivo mientras los hilos trabajan
		while any(th.is_alive() for th in threads):
			time.sleep(0.5)
	except KeyboardInterrupt:
		print("Saliendo por interrupción del usuario…")
...
if __name__ == "__main__":
 ...
 #py s1.py  --ip 192.168.95.194 --ip 192.168.95.195
